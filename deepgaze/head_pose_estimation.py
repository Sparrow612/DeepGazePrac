...
class CnnHeadPoseEstimator:
    ...
    def load_yaw_variables(self, YawFilePath):
        ...
        if not os.path.isfile(YawFilePath): raise ValueError(
    ...
    def return_yaw(self, image, radians=False):
        ...
        if h != w or w < 64 or h < 64:
            if h != w:
                raise ValueError(
                    '[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input has wrong shape. Height '
                    'must equal Width. Height=%d,Width=%d' % (
                        h, w))
            else:
                raise ValueError(
                    '[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input has wrong shape. Height '
                    'and Width must be >= 64 pixel')
        # wrong number of channels
        if d != 3:
            raise ValueError(
                '[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input does not have 3 channels, '
                'this function accepts only colour images.')
    ...
    def _allocate_pitch_variables(self):
        ...
        self.hp_conv3_weights = tf.Variable(
            tf.random.truncated_normal([3, 3, 128, 256], stddev=0.1))  # was[3, 3, 128, 256]
        ...
        def model(data):
            ...
            X = tf.reshape(data, shape=[-1, 64, 64, 3])
            if DEBUG: print("SHAPE X: " + str(X.get_shape()))

            # Convolution Layer 1
            conv1 = tf.tanh(tf.nn.bias_add(tf.nn.conv2d(X, self.hp_conv1_weights, strides=[1, 1, 1, 1], padding='SAME'),
                                           self.hp_conv1_biases))
            if DEBUG: print("SHAPE conv1: " + str(conv1.get_shape()))
            # Max Pooling (down-sampling)
            pool1 = tf.nn.max_pool2d(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            if DEBUG: print("SHAPE pool1: " + str(pool1.get_shape()))
            # Apply Normalization
            norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)
            # Apply Dropout
            # norm1 = tf.nn.dropout(norm1, _dropout)
            ...
            if DEBUG: print("SHAPE conv2: " + str(conv2.get_shape()))
            # Max Pooling (down-sampling)
            pool2 = tf.nn.max_pool2d(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            if DEBUG: print("SHAPE pool2: " + str(pool2.get_shape()))
            # Apply Normalization
            norm2 = tf.nn.lrn(pool2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)
            # Apply Dropout
            # norm2 = tf.nn.dropout(norm2, _dropout)
            ...
            if DEBUG: print("SHAPE conv3: " + str(conv3.get_shape()))
            # Max Pooling (down-sampling)
            pool3 = tf.nn.max_pool2d(conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            if DEBUG: print("SHAPE pool3: " + str(pool3.get_shape()))
            # Apply Normalization
            norm3 = tf.nn.lrn(pool3, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

            # Fully connected layer 4
            dense1 = tf.reshape(norm3, [-1, self.hp_dense1_weights.get_shape().as_list()[0]])  # Reshape conv3
            if DEBUG: print("SHAPE dense1: " + str(dense1.get_shape()))
            ...
            if DEBUG: print("SHAPE out: " + str(out.get_shape()))
            return out
    ...
    def return_pitch(self, image, radians=False):
        ...
        if h != w or w < 64 or h < 64:
            raise ValueError(
                '[DEEPGAZE] CnnHeadPoseEstimator(return_pitch): the image given as input has wrong shape. Height and '
                'Width must be >= 64 pixel')
        # wrong number of channels
        if d != 3:
            raise ValueError(
                '[DEEPGAZE] CnnHeadPoseEstimator(return_pitch): the image given as input does not have 3 channels, '
                'this function accepts only colour images.')
